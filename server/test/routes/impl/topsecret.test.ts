...
const documentClient: IMock<DocumentClient> = Mock.ofType<DocumentClient>();
 ...
 */
test('Document client is called with correct cognito identity', () => {
    headers.setup(mock => mock.cognitoIdentityId).returns(() => 'test')

    controller.getFavoriteDog(expressRequest.object, expressResponse.object);
